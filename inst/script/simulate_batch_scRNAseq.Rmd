...
batch_var_list <- c(0.01, 0.1, 0.3, 0.5)
batch_diff_list <- c(0.3)

any <- lapply(batch_diff_list, function(diff){
  batch_list <- lapply(batch_var_list, sim_multibatch_data, n.genes = 300, n.batch = 3, batch_diff = diff, n_sample = 500)
  names(batch_list) <- c("batch1", "batch10", "batch30", "batch50")
  batch_list
})
...
lapply(batch_list, function(name){
    sce_name <- sce_list[["unbalanced30"]][[name]]
    visGroup(sce_name, "batch") + ggtitle(paste0("unbalanced30_", name))
})
...
add_mnn <- function(batch_name){
  sce <- sce_list[["unbalanced30"]][[batch_name]]
...
lapply(batch_list, function(name){
    sce <- sce_mnn[[name]]
    visGroup(sce, "batch", dim_red = "MNN") + ggtitle(name)
})
```
...
sim_30 <- sce_mnn
cms_sim30 <- cms(sce_mnn[[1]][,c(1:50, 500:550)], k = 30, group = "batch")
save(sim_30, file = paste0(data_path,"/sim30.rda"))
save(cms_sim30, file = paste0(data_path,"/cms_sim30.rda"))
sessionInfo()
```
