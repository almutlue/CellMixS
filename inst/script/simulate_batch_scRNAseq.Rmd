...
batch_var_list <- c(0, 0.1, 0.3, 0.5)
batch_diff_list <- c(0.5)

any <- lapply(batch_diff_list, function(diff){
  batch_list <- lapply(batch_var_list, sim_multibatch_data, n.genes = 300, n.batch = 3, batch_diff = diff, n_sample = 400)
  names(batch_list) <- c("batch0", "batch10", "batch30", "batch50")
  batch_list
})
...
lapply(batch_list, function(name){
    ...
    visGroup(sce_name, "batch", dim_red = "UMAP") + ggtitle(paste0("unbalanced50_", name))
})
