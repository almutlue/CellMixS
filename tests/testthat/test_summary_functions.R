...
test_that("test output of cms_summary",{
  ...
  summary_res <- cmsSummary(cms_smooth)
  summary_raw <- cmsSummary(cms_raw)
  summary_batch <- cmsSummary(cms_smooth, sum_var = "batch", sce = sce)
...
test_that("test that compareIntegration and compareCluster work",{
  ...
  cms_list <- list("raw"= cms_smooth[,"cms"], "mnn" = cms_mnn[,"cms"])
  ...
  cms_group <- data.frame("cms"= cms_smooth[,"cms"],
                          "group" = sample(c("A", "B", "C"), nrow(cms_smooth), replace = TRUE))
  compare_group <- compareCluster(cms_smooth, "group", cms_var = "cms", sce = sce, violin = TRUE)
  compare_group2 <- compareCluster(data.frame("cms" = cms_smooth[,"cms"]), "batch", sce = sce)
