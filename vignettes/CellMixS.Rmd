...
The package contains example data named **sim_30**, a list of simulated single cell RNA-seq data with varying batch effect strength and unbalanced batch sizes. 
...
All datasets consist of *3 batches*, one with _500 cells_ and the others with _150 cells_. The simulation is modified after [@Buttner2019] and described in [sim30](https://github.com/almutlue/CellMixS/blob/master/inst/script/simulate_batch_scRNAseq.Rmd).
...
cms_sce10 <- visMetric(cms_res = cms10, sce = sce10, cms_var = "cms_smooth")
...
visCluster(cms10, "celltype", sce = sce10)
```
...
# plot results
visIntegration(cms_res, violin = TRUE)
...
lim_plot <- visMetric(ldf_lim, sce10, cms_var = "diff", dim_red = "TSNE") + ggtitle("ldfDiff - limma")

mnn_plot <- visMetric(ldf_mnn, sce10, cms_var = "diff", dim_red = "MNN") + ggtitle("ldfDiff - mnn")
...
visIntegration(ldf_res) + labs(title="Summarized ldfDiff",x="method", y = "diff")
