...
It also provides implementations and wrapper for a set of metrics with a similar
purpose as Entropy, Inverse Simpson Index [@Korsunsky2018], Seurat's mixing metric
...
`r BiocStyle::Biocpkg("CellMixS")` can be installed from Bioconductor as following.
...
half of its size (so  _125 cells_). The simulation is modified after 
...
visualization (e.g. in a normal tSNE or UMAP plot) depending on the strength. `r BiocStyle::Biocpkg("CellMixS")` has different plotting functions to 
...
distribution of cells. The `cms` function takes an `SingleCellExperiment` 
...
A key question when evaluating datset structures is how to define neighbourhoods
...
parameter is provided. It defines the minimum number of cells that define a 
...
In this case the neighbourhoods will include as many neighbours until from all 
batches at least `batch_min` cells are included.


## Further cms parameter setting
...
`isi` calculates the Inverse Simpson index within each cells *knn*. 
