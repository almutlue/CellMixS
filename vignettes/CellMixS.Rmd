...
`r BiocStyle::Githubpkg('almutlue/CellMixS')` can be installed from GitHub using 
the `r BiocStyle::CRANpkg("devtools")` package from CRAN.
...
Batch effects were introduced by sampling 0%, 10%, 30% or 50% of gene expression 
values from a distribution with variant mean (e.g. 1% - 50% of genes were 
...
#cms only cms10
sce15 <- sim_list[["batch15"]]
metric_plot <- visMetric(sce15, metric_var = "cms_smooth.unaligned")

#group only cms10
group_plot <- visGroup(sce15, group = "batch")
...
sce15[["celltype"]] <- sample(c("CD4+", "CD8+", "CD3", "CD19"), 
                              ncol(sce15), replace = TRUE)

visOverview(sce15, "batch", other_Var = "celltype")
```

Systematic differences (e.g. celltype differences) can be further explored using `visCluster`. Here we do not expect any systematic difference as celltypes were 
randomly assigned.
```{r compare cluster, fig.small= TRUE}
visCluster(sce15, metric_var = "cms.unaligned", 
...
reducedDimNames(sce15)
sce15 <- cms(sce15, k = 70, group = "batch", 
             dim_red = "MNN", res_name = "MNN")

# run limma
limma_corrected <- removeBatchEffect(counts(sce15), batch = sce15$batch)
#add corrected counts to sce
assay(sce15, "lim_corrected") <- limma_corrected 
...
To compare different methods summary plots from `visIntegration` and p-value 
histograms from `visHist` can be used. Local patterns within single methods can 
be explored as described above.
```{r batch correction methods vis}
# plot results
visIntegration(sce15, metric_prefix = "cms.", violin = TRUE)

# As pvalue histograms
visHist(sce15, metric_prefix = "cms.",  n_col = 3)
```

Here both methods `r BiocStyle::Biocpkg("limma")` and `fastMNN` from the `r BiocStyle::Biocpkg("scran")` package show a flat p.value distribution. So cells 
are equally mixed after batch effect removal.
...
sce_pre_list <- list("1" = sce15[,sce15$batch == "1"], 
                     "2" = sce15[,sce15$batch == "2"], 
                     "3" = sce15[,sce15$batch == "3"])

sce15 <- ldfDiff(sce_pre_list, sce_combined = sce15, 
                 group = "batch", k = 70, dim_red = "PCA", 
                 dim_combined = "MNN", assay_pre = "counts", 
                 n_dim = 20, res_name = "MNN")

sce15 <- ldfDiff(sce_pre_list, sce_combined = sce15, 
                 group = "batch", k = 70, dim_red = "PCA", 
                 dim_combined = "PCA", assay_pre = "counts", 
                 assay_combined = "lim_corrected",  
                 n_dim = 20, res_name = "limma")
...
lim_plot <- visMetric(sce15, metric_var = "diff_ldf.limma", 
                      dim_red = "TSNE") 

mnn_plot <- visMetric(sce15, metric_var = "diff_ldf.MNN", 
                      dim_red = "MNN") 

plot_grid(lim_plot, mnn_plot, ncol = 2)

#ldfDiff score summarized
visIntegration(sce15, metric_prefix = "diff_ldf") 

```

`ldfDiff` shows a clear difference between both methods. While `r BiocStyle::Biocpkg("limma")` is able to preserve the batch internal structure 
within batches, `fastMNN` clearly changes it. Even if batches are well mixed 
(see \@ref(di2)), `fastMNN`does not work for batch effect removal on these 
simulated data.  
