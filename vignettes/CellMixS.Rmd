...
All datasets consist of *3 batches*, one with _400 cells_ and the others with half of its size (so  _200 cells_). The simulation is modified after [@Buttner2019] and described in [sim50](https://github.com/almutlue/CellMixS/blob/master/inst/script/simulate_batch_scRNAseq.Rmd).
...
batch_names <- c("batch0", "batch10", "batch30")
...
cms_mat <- batch_names %>% map(function(name) sim_list[[name]]$cms.unaligned) %>% bind_cols() %>% set_colnames(batch_names)


visHist(cms_mat, n_col = 5)
...
#cms only cms10
sce10 <- sim_list[["batch10"]]
metric_plot <- visMetric(sce10, metric_var = "cms_smooth.unaligned")

#group only cms10
group_plot <- visGroup(sce10, group = "batch")
...
visOverview(sce10, "batch", other_Var = "celltype")
```
...
visCluster(sce10, metric_var = "cms.unaligned", cluster_var = "celltype")
```
...
#run cms
sce10 <- cms(sce10, k = 80, group = "batch", assay_name = "lim_corrected", res_name = "limma")
...
To compare different methods summary plots from `visIntegration` and p-value histograms from `visHist` can be used. Local patterns within single methods can be explored as described above.
```{r batch correction methods vis}
# plot results
visIntegration(sce10, metric_prefix = "cms.", violin = TRUE)

# As pvalue histograms
visHist(sce10, metric_prefix = "cms.",  n_col = 3)
```
...
```{r ldfDiff}
#Prepare input 
# list with single SingleCellExperiment objects (Important: List names need to correspond to batch levels! See ?ldfDiff)
sce_pre_list <- list("1" = sce10[,sce10$batch == "1"], "2" = sce10[,sce10$batch == "2"], "3" = sce10[,sce10$batch == "3"])

sce10 <- ldfDiff(sce_pre_list, sce_combined = sce10, group = "batch", k = 70, dim_red = "PCA", dim_combined = "MNN", assay_pre = "counts", n_dim = 20, res_name = "MNN")

sce10 <- ldfDiff(sce_pre_list, sce_combined = sce10, group = "batch", k = 70, dim_red = "PCA", dim_combined = "PCA", assay_pre = "counts", assay_combined = "lim_corrected",  n_dim = 20, res_name = "limma")
...
lim_plot <- visMetric(sce10, metric_var = "diff_ldf.limma", dim_red = "TSNE") 

mnn_plot <- visMetric(sce10, metric_var = "diff_ldf.MNN", dim_red = "MNN") 

plot_grid(lim_plot, mnn_plot, ncol = 2)

#ldfDiff score summarized
visIntegration(sce10, metric_prefix = "diff_ldf") 
