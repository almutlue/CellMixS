...
#'
#' @seealso \code{\link{ad.test}} for Anderson-Darling test on k-samples, \code{\link{cms.pca}} for cms in PCA subspace, \code{\link{scale.cms}} for scaling.
...
#' @param k_min Numeric giving the minimum number of k nearest neighbours to use. cells to include.
#' If k_min is defined the overall distance density distribution of knn will be determined and only cells up to the first local minima that includes more than kmin cells will be used.
#' @param scale A logical value indicating if cms results should be scaled within each neighbourhood.
...
#'
#' @family cms functions
#' @seealso \code{\link{cms.cell}} for cms, \code{\link{scale.cms}} for scaling.
#'
#' @return
#' @export
#'
...
cms.pca <- function(sce, k, group, k_min = NA, scale = TRUE, n_pc = 30, cell_min = 10){
  ...
  if(scale == TRUE){
    res_cms <- scale.cms(knn, cms_raw, cell_names, k_min, k)
  }else{
    res_cms <- cms_raw
  }
 res_cms
...
#'
#' @family cms functions
#' @seealso \code{\link{cms.cell}}, \code{\link{scale.cms}}, \code{\link{cms.pca}}.
#'
#' @return
#' @export
#'
...
#' scale.cms
#'
#' Performs weighted scaling of cms scores
#'
...
#' @param k Numeric, defining the number of k-nearest neighbours to use for testing the "mixing" within the neighbourhood.
#'
#' @details Internal function to scale cms scores if \code{\link{cms.pca}} = TRUE. Replaces cms scores by weigthed means of cms scores within the k-nearest neighbourhood.
#' Reciprocal distances are used as weights. Returns a matrix with "scaled-cms" and original "cms" values.
#'
#' @family cms functions
...
scale.cms <- function(knn, cms_raw, cell_names, k_min, k){
  ...
  # how many cells to scale over
  k_scaled <- ifelse(!is.na(k_min), k_min, k)

  cms_scaled <- do.call(rbind, lapply(cell_names, function(cell_id){
    # add 1 to ensure that distances < 1 are less weighted than the original cell
    weights <- c(1,(1/(knn[[2]][cell_id,c(1:k_scaled)] + 1)))
    knn_cms <- c(cms_raw[cell_id,"cms"],knn[["cms"]][cell_id, c(1:k_scaled)])
    scaled_cms <- weighted.mean(knn_cms, weights)
  }))
  res_cms <- cbind(cms_scaled, cms_raw)
  rownames(res_cms) <- cell_names
  colnames(res_cms) <- c("cms_scaled", "cms")
  res_cms
}
