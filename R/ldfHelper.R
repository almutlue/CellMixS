...
#'
#' @param dataset Matrix with cell embeddings with cells as rows and reduced dimensions as cloumns.
#' Subspace to determine LDF in.
#' @param knn_object List with k-nearest neighbours (KNN) as provided by \code{get.knn} from the \code{FNN} package.
...
#' The  greater  the  bandwidth, the smoother kernels and lesser weight are put on outliers. Default is 1
...
#' Calculates a Local Density Estimate (LDE) and Local Density Factor (LDF) with a gaussian kernel.
#' Modified to use a predefined KNN neighbourhood.
#' For \code{\link{ldfSce}} this is essential to determine LDF after data integration on the same set of cells.
#'
...
#'
#' @param sce A \code{SingleCellExperiment} object with the data to define the subspace.
#' @param assay_name Character. Name of the assay to use for PCA. Only relevant if no existing 'dim_red' is provided.
#' Must be one of \code{names(assays(sce))}.
#' @param dim_red Character. Name of embeddings to use as subspace.
#' @param n_dim Numeric. Number of subspace elements to include to define subspace.
#'
#' @details Function to determine the subspace for \code{ldfDiff} and \code{cms}.
#' Checks whether the defined 'dim_red' is present. Only if no subspace is defined or present it will perform a PCA using \code{runPCA}.
#' To calculate PCA counts defined in 'assay_name' are used.
#'
#' @family helper functions
#' @seealso \code{\link{ldfSce}}, \code{\link{cms}}.
#'
#' @return A matrix of cell embeddings with cells as rows and reduced dimensions as columns.
#'
